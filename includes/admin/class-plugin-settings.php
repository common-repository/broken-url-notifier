...
class Broken_Url_Notifier_Admin_Options {
    private $page_hook = '';
    public $settings;
    ...
    function  admin_menu() {
		$this->page_hook = add_submenu_page('broken-url-notifier',
											__('Settings Page',BUN_TXT),
											__('Settings Page',BUN_TXT),
											'administrator','broken-url-notifier', array( $this, 'admin_page' ) );
		//var_dump($this->page_hook); exit;
    ...
    private function add_settings_pages(){
        global $pages;
        $pages =  array();
        include(BUN_ADMIN.'settings/pages.php');
    ...
    private function add_settings_section(){
        global $section;
        $section =  array();
        include(BUN_ADMIN.'settings/section.php');
            ...
            if(is_array($s)){
                ...
                while($a < $c){
                    if(isset($s[$a]['validate_callback'])){
                        ...
                        $file = addslashes(BUN_ADMIN.'settings/validate-'.$s[$a]['id'].'.php');
    ...
    private function add_settings_fields(){
        global $fields;
        $fields =  array();
        include(BUN_ADMIN.'settings/fields.php');
        $this->settings_field = $fields;
    }
 
	function admin_screen($hook){
		if($this->page_hook == $hook->id){
			//$this->settings->init( $this->pages, BUN_DBS );
		}
	}

    function admin_init(){
		$this->settings = new Broken_Url_Notifier_Settings();
		...
		$this->settings->init($pages, BUN_DBS );
    }
    ...
    function get_settings($key = ''){
        ...
        foreach($this->settings_page as $settings){
            $this->settings_key[] = BUN_DB.$settings['slug'];
            $db_val = get_option(BUN_DB.$settings['slug']);
            if(is_array($db_val)){
